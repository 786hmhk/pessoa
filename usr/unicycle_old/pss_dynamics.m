...
x=params_symb.eta*(xl+params_symb.min(params_symb.xoind)); %x(1)=x, x(2)=y, x(3)=\theta

%% Here computations on states

% Some previous computations and other arrangements neccessary
minpoint_label=zeros(params_symb.n,1);
...
ntrans=1;
jl=1;
while(jl<=params_symb.n && x(params_symb.n)<=maxpoint_label(params_symb.n))
jl=1;
    xend=[xend x];
    while(jl<params_symb.n && x(jl)>=maxpoint_label(jl))
