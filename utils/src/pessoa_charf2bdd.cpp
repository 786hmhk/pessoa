                  ...
                  int nrhs, const mxArray *prhs[] )
{
	...
	//build_charf_BDDW returns number of states
	ntarget_states=build_charfBDDW(ddman,ddnodearray,BDD_W,wmin,wmax,&params_symb, nbatch);
	...
	if(verbose==3){
		fp=fopen("dumpfile_charf2bdd.dot", "w");
	...
	if(verbose==2)
	{
		...
		if(plot_ok == -1)
			mexPrintf("Plotting set stopped... \n");
	}
